...
4. Run for real. These are GPU jobs that can take minutes (baselines/roi) to
   ...
   comparison methodology in AGENTS.md needs). A later plain
...
6. Analyze against the experiment's comparison target per the
   "Evaluation methodology" section in AGENTS.md (foreground-first, matched
...
- Don't touch `src/presley/cli.py` / `pipeline_legacy.py` — see root AGENTS.md.
