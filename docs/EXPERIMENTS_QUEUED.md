...
| **Q4** | Real-HAT-GAN vs Real-ESRGAN | **DONE 2026-07-28.** Hashes bear `661d092654b4e18d` / camel `4ec11482c4cfc93e` (fp32, tile=400). vs Real-ESRGAN `e2cb6bed165d69b1` / `c29c94b5c290f208`: FG indistinguishable; BG-LPIPS within JND both videos (bear HAT 0.201 vs 0.223 = 0.46×JND; camel HAT 0.160 vs 0.152 = 0.15×JND). Keep Real-ESRGAN; Real-HAT = recent twin (like BSRGAN). **fp16 banned** (Softmax NaNs). | Recent SR GAN |
...
Q1–Q5 done. Next authorized work (human-gated):
1. Q6 DiffBIR — only with a new mechanism argument (NAFNet already ties unsharp)
2. Stream-DiffVSR / DC-VSR (Q7/Q8) when downsample diffusion story is needed
3. Q9 / Q10 as bandwidth allows
4. Write-up-only D6 tables / Overleaf push of paper local commits — only if asked
