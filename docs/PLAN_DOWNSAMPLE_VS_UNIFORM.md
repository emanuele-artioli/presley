...
## 6. RESULT (2026-08-06) — interpretation (B), and weaker than (B) anticipated

**Outcome: no winner is established.** Selective wins FG BD-rate on 2 of 6
videos (`bear` -18.3%, `india` -30.8%) and loses on 4 (`camel` +60.2%, `tennis`
+60.6%, `pigs` +86.4%, `dog` +89.8%). Exact two-tailed sign **p = 0.6875** on
FG, **0.2188** on BG. At n=6 the attainable floor is p=0.0312, so a 4-2 split
**cannot** be significant at any threshold. This is a split, not a defeat and
not a win.

| video | BD-rate FG | BD-rate BG | d FG-LPIPS | d bits @QP | overlap | n@QP |
|---|---|---|---|---|---|---|
| bear   | **-18.3%** | **-10.4%** | +0.0869 | -44.1% | 0.83 | 3 |
| camel  | +60.2% | +21.7% | -0.0012 | -33.0% | 0.67 | 4 |
| dog    | +89.8% | +55.5% | -0.0324 | -31.1% | 0.62 | 4 |
| india  | **-30.8%** | +40.5% | +0.1005 | -25.6% | 0.67 | 4 |
| pigs   | +86.4% | +51.8% | -0.0222 | -32.6% | 0.60 | 4 |
| tennis | +60.6% | +65.6% | -0.0267 | -28.5% | 0.65 | 4 |

### Bounds that fired

- **FG BD-rate breached the +50% alarm on 4 of 6** (`camel`, `dog`, `pigs`,
  `tennis`), against a pre-registered plausible worst case of +25%. The band was
  too optimistic; it is **recorded as fired and NOT re-fitted**.
- **d FG-LPIPS fell just below its -0.02 worst case on 3** (`dog` -0.0324,
  `tennis` -0.0267, `pigs` -0.0222) — all well inside the -0.05 alarm, and all
  sub-JND, so "no perceptible FG difference" is the correct wording there.
- **Mechanism gate PASSED on all 24**: coverage exactly 1.0, single strength
  value, `rate_control=crf`, empty `invariant_failures`.
- **d bits @QP in band on 5 of 6** (-25.6% to -33.0%); `bear` at -44.1% is
  inside the -15..-60% expectation. Uniform does code cheaper, as predicted.

### Two analysis bugs found and fixed before any of this was believed

1. **Pairing by config query returned a mixture of arms.** Up to 21 runs share
   the selective key across freeze/blur/noise/mean_fill and a dozen restorers.
   The first pass produced a non-monotonic rate ladder and **+711% BD-rate on
   bear**. Selective hashes are now pinned from section 3.1; a non-monotonic
   ladder aborts.
2. **BD-rate integrated over barely-overlapping curves.** `overlap_fraction`
   ships for exactly this and was not called. Curves below 0.50 overlap are now
   excluded, not averaged in.

### POST-HOC observation — hypothesis-generating only, NOT evidence

Selective wins **iff** uniform's foreground damage is supra-JND: 6/6 concordant
(`bear` +0.087 and `india` +0.101 are the two supra-JND cases, and the two
selective wins). This was **not** pre-registered — it was generated by these
data — so quoting its p-value would be candidate shopping. It needs an
independent set before it can be claimed.

**FG area is refuted again**, a third independent time: `india` 0.325 wins while
`pigs` 0.327 loses; `bear` 0.114 wins while `dog` 0.090 loses.

### Scope limit that must travel with any text

The uniform arm transmits full-resolution-but-lowpassed frames, **not** a
genuine lower-rendition bitstream with fewer coded pixels. A real rendition
ladder codes fewer pixels and would likely do *better* still, so this comparison
is a **conservative proxy** for industry practice. It answers "does selectivity
beat uniformity with the transport held identical" — not "does PRESLEY beat
Netflix's ladder."
