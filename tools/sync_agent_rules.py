...
This script maintains everything that cannot be hand-written:
    ...
    AGENTS.md `host-rules` block   a pointer at ~/.agent-rules/AGENTS.md
    .cursor/rules/cursor-harness.mdc  a pointer at the host rules + harness
...
Why the host rules are a pointer rather than a copy: a copy is a second source
of truth that goes stale without saying so. Three projects were found five
weeks behind on 2026-08-31, still carrying a git rule the host had replaced.
Claude Code and Antigravity load `~/.agent-rules/AGENTS.md` themselves; every
other reader gets the path. The cost is real and accepted: Copilot's and
Cursor's cloud agents, and CI runners, never see this host's home directory and
so no longer see the host rules at all. `AGENTS.md` states that acceptance
explicitly. Do not reintroduce a copy "just for cloud" without changing the
host file's decision first.
...
def host_block() -> str | None:
    """A **pointer** to the host rules, not a copy of them.

    This used to inline the whole of `~/.agent-rules/AGENTS.md`. The copy was a
    second source of truth pretending not to be one: an edit to the host file
    was stale in every project until somebody remembered to re-run this script,
    and on 2026-08-31 three projects were found five weeks and one rewrite
    behind — still telling their agents "never destroy work you have not read"
    after the host had replaced that with a policy giving agents autonomy over
    anything revertible. Nothing warned; the file simply read plausibly.

    **The tradeoff, decided rather than drifted into.** The copy existed for
    agents that cannot see this host's home directory — Copilot's and Cursor's
    cloud agents, and CI runners. Those now get the pointer and nothing else.
    That is the gap `AGENTS.md` already accepts in as many words ("Cloud agents
    on other machines will not see it; accepted"), because the rules describe a
    shared GPU server that cloud agents are not working on. Do not reintroduce
    a copy "just for cloud" without changing that decision in the host file
    first, or this whole failure mode comes back.
    """
    ...
    return (
        ...
        f"mechanics are beside it in `{HOST_DIR}/harness/`.\n\n"
        f"@{HOST_RULES}\n\n"
        "<!-- host-rules:end -->"
    )
...
def cursor_rule(_harness: str) -> str:
    """A pointer to the host's Cursor harness file, not a copy of it.

    Cursor has no user-level rules file, so *something* must be delivered per
    project — but it does not have to be the prose. An `alwaysApply` rule that
    names the path is enough for a session on this host, and it cannot go
    stale. The argument is kept for signature compatibility with callers that
    still read the harness file to decide whether to write this at all.
    """
    return (
        "---\n"
        "description: Host rules and Cursor harness — pointers, not copies\n"
        "alwaysApply: true\n"
        "---\n\n"
        "<!-- GENERATED by tools/sync_agent_rules.py — DO NOT EDIT. -->\n\n"
        "Host-wide rules and Cursor-specific mechanics live on this machine,\n"
